...
def simulate_ramp_scene(scene: Dict, total_time_s: float = 2.0, dt_s: float | None = None) -> Dict:
    """Analytic motion for a block on an inclined ramp.

    Assumptions:
    - Single mass m1 moves along ramp axis, angle theta from horizontal.
    - Acceleration a = g*sin(theta) - mu_k*g*cos(theta) (clamped to >= 0).
    - Positive direction is downslope.
    - Positions reported in world (x,y) using initial position as origin for displacement.
    """
    world = scene.get("world", {})
    g = float(world.get("gravity_m_s2", 9.81))
    dt = float(world.get("time_step_s", 0.016)) if dt_s is None else float(dt_s)
    ...
    if not b1:
        return {"frames": [], "energy": {}}
    x0, y0 = b1.get("position_m", [0.0, 0.0])
    # Constraint params
    angle_deg = 30.0
    mu_k = 0.0
    try:
        cons = (scene.get("constraints", []) or [{}])[0]
        if "angle_deg" in cons:
            angle_deg = float(cons.get("angle_deg", angle_deg))
        if "mu_k" in cons:
            mu_k = float(cons.get("mu_k", 0.0))
    except Exception:
        pass
    theta = math.radians(angle_deg)
    a = g * math.sin(theta) - mu_k * g * math.cos(theta)
    if a < 0:
        a = 0.0
    steps = max(1, int(total_time_s / dt))
    frames: List[Dict] = []
    # Unit vector along ramp downslope
    ...
    for i in range(0, steps + 1):
        ...
        px = float(x0 + ux * s)
        py = float(y0 + uy * s)
        frames.append({"t": round(t, 5), "positions": {"m1": [px, py]}})
    return {"frames": frames, "energy": {}}

__all__.append("simulate_ramp_scene")
