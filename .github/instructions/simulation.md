...
시뮬레이션 박스에서:
1. **객체 클릭** → Simulation Controls 패널에 해당 객체의 파라미터 표시
2. **파라미터 조절** → Position, Size, Mass, Friction 등을 실시간 변경
3. **객체 드래그** → 마우스로 위치 조절
4. **Constraint 인식** → Rope 등으로 연결된 객체는 함께 움직임
5. **Backend 동기화** → 모든 변경사항이 자동으로 저장 및 재시뮬레이션
