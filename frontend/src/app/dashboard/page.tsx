...
function PaneShell({ title, hint, children, className, bodyClassName, headerClassName }: PaneShellProps) {
    return (
        <section className={cn('flex h-full min-h-0 flex-col bg-background', className)}>
            <div className={cn('flex items-center justify-between border-b px-4 py-3', headerClassName)}>
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        const handleChange = () => setShouldStackPanels(mediaQuery.matches);
        ...
        const legacyListener = () => handleChange();
            ...
            <GlobalChatProvider>
                <div className="flex h-screen flex-col overflow-hidden bg-muted/10">
                    {shouldStackPanels ? (
                        <ResizablePanelGroup
                            direction="vertical"
                            className="flex-1 min-h-0 overflow-hidden border-t bg-background"
                        >
